...
export default function paginationField(){
  return {
    ...
    read(existing = [], { args, cache }){
      ...
      //if there are items, just retur them from the cache, and we don't need to go to the network
      if(items.length) {
        console.log(`There are ${items.length} items in the cache! Gonna send them to apollo`);
        return items
      }

      return false //fallback to network if the if-statements don't work out
